...
LRLIST=(1 0.5 0.25 0.125 0.0625 0.03125 0.015625 0.0078125 0.00390625 0.001953125)
...
for u in ${LRLIST[@]}; do
    for ((n=1; n<=MAXN; n++)); do
	    ...
	    echo "[$(date)] Group $i" 
            for ((j=0; j<PAR; j++)); do
                s=$((MINSEED + i * PAR + j))
		/usr/bin/time -f "User Time: %U\nSystem Time: %S" python "${DIRNAME}/../solvers/sgd.py" -n $n --seed $s --lr $u &> "results_sgd/sgd_n${n}_s${s}_u${u}.txt" &
                pidList[${#pidList[@]}]=$!
            done
